...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_stan", "queries")
...
class EggInfo(egg_info):
    def find_sources(self):
        ...
        self.filelist.include("grammars/stan/src/tree_sitter/*.h")
...
setup(
    ...
    package_data={
        "tree_sitter_stan": ["*.pyi", "py.typed"],
        "tree_sitter_stan.queries": ["*.scm"],
    },
    ext_package="tree_sitter_stan",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_stan/binding.c",
            ...
            include_dirs=["grammars/stan/src"],
